  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pt-32 pb-20 px-4">
      <div className="max-w-7xl mx-auto text-center">
        ...
        {(sai || ai) && (
          <Reveal delay={0.1} className="flex flex-col sm:flex-row justify-center gap-4 mb-16 max-w-2xl mx-auto">
            {sai && (
              <div className="flex-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl p-5 flex items-center gap-4 text-left">
                ...
                <div>
                  ...
                  <h3 className="text-lg font-black text-slate-900 dark:text-white uppercase italic leading-tight">{sai.name}</h3>
                </div>
              </div>
            )}
            {ai && (
              <div className="flex-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl p-5 flex items-center gap-4 text-left">
                ...
                <div>
                  ...
                  <h3 className="text-lg font-black text-slate-900 dark:text-white uppercase italic leading-tight">{ai.name}</h3>
                </div>
              </div>
            )}
          </Reveal>
             ...
             <div className="flex flex-col gap-4 w-full">
               <div className="bg-white dark:bg-slate-900 border border-yellow-500/30 p-6 rounded-2xl w-full transition-all hover:-translate-y-1 text-left flex items-center gap-4">
                   ...
                   <div>
                     ...
                     <h3 className="text-xl font-black text-slate-900 dark:text-white uppercase italic leading-none">{csm.name}</h3>
               ...
               {sgmMembers.length > 0 && (
                 <div className="flex flex-col items-center w-full pl-6 border-l-2 border-slate-300 dark:border-slate-800/60 gap-3 mt-1">
                   {sgmMembers.map((sgmItem, index) => (
                     <div key={index} className="bg-slate-50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 p-4 rounded-xl w-full transition-all hover:-translate-y-1 text-left flex items-center gap-4">
                         ...
                         <div>
                           ...
                           <h3 className="text-md font-black text-slate-700 dark:text-slate-200 uppercase italic leading-none">{sgmItem.name}</h3>
        ...
        <Reveal delay={0.2} className="mt-24 max-w-4xl mx-auto">
          <div className="flex justify-center gap-4 mb-12">
            {['staff', 'company'].map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`px-8 py-3 rounded-full font-black uppercase tracking-widest text-xs transition-all ${activeTab === tab ? 'bg-yellow-500 text-slate-950' : 'bg-slate-100 dark:bg-slate-900 text-slate-500 border border-slate-200 dark:border-slate-800'}`}
              >
          ...
          <AnimatePresence mode="wait">
            {activeTab === 'staff' ? (
                  ...
                  <div key={i} className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 border-l-4 border-l-yellow-600 p-5 flex justify-between items-center group rounded-xl">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <h4 className="text-xl font-bold text-slate-900 dark:text-white italic">{s.name}</h4>
                        <p className="text-xs text-slate-500 font-medium">{s.desc || 'Battalion Staff Officer'}</p>
                      </div>
                    </div>
                    <ShieldCheck className="text-slate-300 dark:text-slate-800 group-hover:text-yellow-600 transition-colors" />
                  </div>
                ))}
              </motion.div>
            ) : (
              <motion.div key="company" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="grid md:grid-cols-2 gap-6">
                {companies.map((co, i) => (
                  <div key={i} className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 text-left border-t-2 border-t-yellow-600/50 relative overflow-hidden group">
                    <h4 className="text-xl font-black text-slate-900 dark:text-white uppercase italic mb-6 flex items-center gap-2">
                      <Users size={18} className="text-yellow-500" />
                      {co.name}
                    </h4>
                    <div className="space-y-4">
                      {co.staff.map((person, idx) => (
                        <div key={idx} className="flex justify-between items-center border-b border-slate-100 dark:border-white/5 pb-2 last:border-0">
                          <div>
                            <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest leading-none mb-1">{person.pos}</p>
                            <p className="text-sm font-bold text-slate-700 dark:text-slate-200 uppercase tracking-tight">{person.name}</p>
                          </div>
